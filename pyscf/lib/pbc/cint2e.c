                  ...
                  double *cache)
{
        ...
        char *shell_in_cell0 = envs_bvk->ovlp_mask + nbas_bvk * nbas_bvk;
        double *qcond_ijij = vhfopt->q_cond;
        double *qcond_iijj = vhfopt->q_cond + Nbas * Nbas;
                  ...
                  CVHFOpt *vhfopt, CINTEnvVars *envs_cint, BVKEnvs *envs_bvk)
{
        ...
        int has_value = PBCint2e_loop(gctr, cell0_shls, bvk_cells, cutoff,
                                      vhfopt, envs_cint, envs_bvk, cache);
        if (has_value) {
