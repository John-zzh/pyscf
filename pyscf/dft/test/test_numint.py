...
class KnownValues(unittest.TestCase):
    def test_make_mask(self):
        ...
        self.assertAlmostEqual(lib.fp(non0), -2.6880474684794895, 9)
        self.assertAlmostEqual(lib.fp(numpy.cos(non0)), 2.5961863522983433, 9)
    ...
    def test_uks_vxc(self):
        ...
        nao = h2o.nao_nr()
        dms = numpy.random.random((2,nao,nao))
        dms = dms + dms.transpose(0, 2, 1) + h2o.intor('int1e_ovlp')
        grids = dft.gen_grid.Grids(h2o)
        v = mf._numint.nr_vxc(h2o, grids, 'B88,', dms, spin=1)[2]
        self.assertAlmostEqual(lib.fp(v), -13.271412906870887, 8)
    ...
    def test_uks_vxc_high_cost(self):
        ...
        nao = mol.nao_nr()
        dms = numpy.random.random((2,nao,nao))
        dms = dms + dms.transpose(0, 2, 1) + mol.intor('int1e_ovlp')
        v = mf._numint.nr_vxc(mol, mf.grids, 'B88,', dms, spin=1)[2]
        self.assertAlmostEqual(lib.fp(v), -6.938074365745656, 8)
    ...
    def test_cache_xc_kernel(self):
        ...
        self.assertAlmostEqual(numpy.einsum('g,g, g->', mf.grids.weights, rho[0], fxc[0]), -15.939767, 2)
